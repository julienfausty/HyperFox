...
def potentiali(t, p, Ax, Ay, p0):
    return (np.exp(-(Ax**2.0 - Ay**2.0)*t)*(np.exp(Ax*(p[0] - p0[0])) + np.exp(-Ax*(p[0] - p0[0])))*np.sin(Ay*(p[1] - p0[1])))

def gradPotentiali(t, p, Ax, Ay, p0):
    res = []
    res.append(Ax*np.exp(-(Ax**2.0 - Ay**2.0)*t)*(np.exp(Ax*(p[0] - p0[0])) - np.exp(-Ax*(p[0] - p0[0])))*np.sin(Ay*(p[1] - p0[1])))
    res.append(Ay*potentiali(t, p, Ax, Ay, p0))
    return res

def potential(t, p, Ax0, Ay0, Ax1, Ay1, p0):
    return (potentiali(t, p, Ax0, Ay0, p0) + potentiali(t, p, Ax1, Ay1, p0))


def gradPotential(t, p, Ax0, Ay0, Ax1, Ay1, p0):
    grads = [gradPotentiali(t, p, Ax0, Ay0, p0), gradPotentiali(t, p, Ax1, Ay1, p0)]
    return [grads[0][0] + grads[1][0], grads[0][1] + grads[1][1]]
...
def analyticalSol(t, p):
    ...
    p0 = [0.5, 0.5]
    D = 1e-1
    return ((-2.0*D/potential(t, p, Ax0, Ay0, Ax1, Ay1, p0))*np.array(gradPotential(t, p, Ax0, Ay0, Ax1, Ay1, p0)))
...
dirName = '../results/Burgers/CG/BEuler/'

subDirName = 'regression_dim-2_h-' + meshSize + '_ord-' + order + '_dt-' + timeStep + '/'
