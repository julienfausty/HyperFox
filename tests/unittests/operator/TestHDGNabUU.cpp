...
TEST_CASE("Testing HDGNabUU operator", "[unit][operator][nonlinear][HDGNabUU]"){
  ...
  for(int i = 1; i < maxDim; i++){
    for(int j = 0; j < maxOrd; j++){
      for(int n = 0; n < (2*(j+1)+1)/3; n++){
        SECTION("Testing HDGNabUU assembly in reference element: (dim = " + std::to_string(i+1) + ", order = " + std::to_string(j+1) + ", n= " + std::to_string(n+1) + ")"){
          if((n+1) % 2 != 0){
