...
TEST_CASE("Testing RungeKutta class", "[unit][operator][TimeScheme][RungeKutta]"){
  ...
  for(int i = 0; i < maxDim; i++){
    for(int j = 0; j < maxOrd; j++){
      ...
      SECTION("Testing Backward Euler (dim="+std::to_string(i+1)+", order="+std::to_string(j+1)+")"){
        ReferenceElement refEl(i+1, j+1, "simplex");
        RungeKutta ts(&refEl, BEuler);
        Mass mass(&refEl);
        ...
        anaVec += (*(mass.getMatrix()))*EMap<const EVector>(fm["OldSolution"].data(), refEl.getNumNodes()) + tStep*EVector::Constant(refEl.getNumNodes(), 1.0);
        ...
        CHECK(ts.getStage() == 0);
        for(int k = 0; k < refEl.getNumNodes(); k++){
          CHECK(fieldMap["Solution"]->getValues()->at(k) - (fieldMap["OldSolution"]->getValues()->at(k) + tStep*fieldMap["RKStage_0"]->getValues()->at(k)) < tol);
        }
        for(int k = 0; k < ts.getNumStages(); k++){
          delete rkStages[k];
        }
      ...
      SECTION("Testing Forward Euler (dim="+std::to_string(i+1)+", order="+std::to_string(j+1)+")"){
        ReferenceElement refEl(i+1, j+1, "simplex");
        RungeKutta ts(&refEl, FEuler);
        Mass mass(&refEl);
        Convection conv(&refEl);
        mass.allocate(1);
        conv.allocate(1);
        ...
        anaVec = (*(mass.getMatrix()) - tStep*(*(conv.getMatrix())))*EMap<const EVector>(fm["OldSolution"].data(), refEl.getNumNodes()) + tStep*EVector::Constant(refEl.getNumNodes(), 1.0);
        EMatrix testMat = anaMat-buffMat;
        EVector testVec = anaVec-buffVec;
        CHECK((testMat.transpose()*testMat).sum() < tol);
        CHECK(testVec.dot(testVec) < tol);
      ...
      SECTION("Testing Crank Nicolson (dim="+std::to_string(i+1)+", order="+std::to_string(j+1)+")"){
        ...
        ts.apply(&buffMat, &buffVec);
        anaMat = (*(mass.getMatrix()));
        anaVec = -(*(conv.getMatrix()))*EMap<const EVector>(fm["OldSolution"].data(), refEl.getNumNodes()); 
        anaVec += EVector::Constant(refEl.getNumNodes(), 1.0);
        anaVec *= tStep;
        anaVec += anaMat*EMap<const EVector>(fm["OldSolution"].data(), refEl.getNumNodes());
        EMatrix testMat = anaMat-buffMat;
        EVector testVec = anaVec-buffVec;
        CHECK((testMat.transpose()*testMat).sum() < tol);
        CHECK(testVec.dot(testVec) < tol);
        ...
        CHECK(ts.getStage() == 2);
        for(int k = 0; k < refEl.getNumNodes(); k++){
          CHECK(fieldMap["RKStage_1"]->getValues()->at(k) == 0.0);
        }
        //compute solution
        ts.computeSolution(&fieldMap);
        CHECK(ts.getStage() == 0);
        for(int k = 0; k < refEl.getNumNodes(); k++){
          CHECK(fieldMap["Solution"]->getValues()->at(k) - (fieldMap["OldSolution"]->getValues()->at(k) + (tStep/2.0)*fieldMap["RKStage_0"]->getValues()->at(k)) < tol);
        }
        for(int k = 0; k < ts.getNumStages(); k++){
          delete rkStages[k];
        }
