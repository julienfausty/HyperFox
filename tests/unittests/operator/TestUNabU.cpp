...
TEST_CASE("Testing UNabU operator", "[unit][operator][nonlinear][UNabU]"){
      ...
      for(int n = 0; n < 2*j/3; n++){
        SECTION("Testing NabUU assembly in reference element: (dim = " + std::to_string(i+1) + ", order = " + std::to_string(j+1) + ", n= " + std::to_string(n+1) + ")"){
          if((n+1) % 2 != 0){
            ...
            CHECK(integral/(2.0*(n+1)) == Approx(TestUtils::intx2n(i+1, (3*n+2)/2)));
            integral = op.getRHS()->dot(u);
            CHECK(integral/(1.0*(n+1)) == Approx(TestUtils::intx2n(i+1, (3*n+2)/2)));
          }
        };
      }
    }
