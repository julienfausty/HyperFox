...
double potentialiBurgersStat(std::vector<double> x, double & Ax, double & Ay, std::vector<double> & x0){
  return ((std::exp(Ax*(x[0] - x0[0])) + std::exp(-Ax*(x[0]-x0[0])))*std::sin(Ay*(x[1] - x0[1])));
};

void potentialiGradBurgersStat(std::vector<double> x, double & Ax, double & Ay, std::vector<double> & x0, std::vector<double> * grad){
  grad->resize(x.size());
  grad->at(0) = Ax * (std::exp(Ax*(x[0] - x0[0])) - std::exp(-Ax*(x[0]-x0[0])))*std::sin(Ay*(x[1] - x0[1]));
  grad->at(1) = Ay * (std::exp(Ax*(x[0] - x0[0])) + std::exp(-Ax*(x[0]-x0[0])))*std::cos(Ay*(x[1] - x0[1]));
};
...
void potentialiHessBurgersStat(std::vector<double> x, double & Ax, double & Ay, std::vector<double> & x0, std::vector<double> * hess){
  ...
  hess->at(1) = Ay * Ax * (std::exp(Ax*(x[0] - x0[0])) - std::exp(-Ax*(x[0]-x0[0])))*std::cos(Ay*(x[1] - x0[1]));
  hess->at(2) = hess->at(1);
...
void runHDGBurgersStat(SimRun * thisRun, HDGSolverType globType){
  ...
  //create buffers  return (potentialiBurgersStat(x, Ax0, Ay0, x0) + potentialiBurgersStat(x, Ax1, Ay1, x0));
  std::vector<int> cell;
